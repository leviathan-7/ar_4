      ...
      switch (augmentedImage.getTrackingState()) {
        ...
        case TRACKING:
          ...
          if (!augmentedImageMap.containsKey(augmentedImage.getIndex())) {
            ...
            float x = camPose.tx() * 10;
            float y = camPose.ty() * 10;

            Pose worldGravityPose = Pose.makeTranslation(x, y, -5f);


            Log.e("camPose_get camera ", camPose.toString(), new Throwable() );

            //camPose = new Pose(transf, camPose.getRotationQuaternion());
            camPose = augmentedImage.getCenterPose().inverse().compose(worldGravityPose);//.compose(camPose);//camPose.inverse().compose(worldGravityPose);



            //Pose mazeGravityPose = augmentedImage.getCenterPose().inverse().compose(worldGravityPose).compose(camPose);
            float mazeGravity[] = camPose.getTranslation();//mazeGravityPose.getTranslation();
            //mazeGravity[1] = 0f;
            //mazeGravity[0] = 0;
            //mazeGravity[2] = 0.5f;

            Log.e("worldGravityPose ", worldGravityPose.toString(), new Throwable() );

            Log.e("camera_pos ", camera_pos.toString(), new Throwable() );

            Log.e("camPose ", camPose.toString(), new Throwable() );
